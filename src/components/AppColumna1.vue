<template>
    ...
    >
      ...
      <div v-for="(task, index) in filteredArrbyStatus" @drop="onDropList($event, task)">
        <TaskItem :task="task" />
      </div>
  ...
  export default {
    computed: {
      ...
      ...mapStores(userStore),
      filteredArrbyStatus() {
    return this.tasksStore.tasks.filter((task) => task.status === this.column.status);
    
  },
    },
    props: ["columnArr", "columnStatus","column"],
    data() {
      return {
        title: "",
        toggleField: false,
    ...
    methods: {
      ...
      async createTask(title, status) {
        ...
        if (this.filteredArrbyStatus.length > 0) {
          orderNum = this.filteredArrbyStatus[this.filteredArrbyStatus.length - 1].order + 1;
        }
